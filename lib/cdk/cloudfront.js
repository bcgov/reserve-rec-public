...
function cloudFrontSetup(scope, props) {
  ...
  const reserveRecOAC = new cloudfront.S3OriginAccessControl(scope, props.env.STACK_NAME + 'OAC', {
    description: 'Reserve Rec Origin Access Control Public (CDK)',
  });

  // Custom Cache Policy to allow Authorization headers
  const cachePolicy = new cloudfront.CachePolicy(scope, props.env.STACK_NAME + 'APICachePolicy', {
  ...
  const reserveRecPublicDistribution = new cloudfront.Distribution(scope, props.env.STACK_NAME + 'PublicDistribution', {
    enabled: true,
  ...
  new CfnOutput(scope, 'Reserve Rec Public Cache Policy', {
    value: cachePolicy.cachePolicyId,
    description: 'Reserve Rec Public Cache Policy Reference',
    exportName: 'ReserveRecPublicCachePolicy',
  });
