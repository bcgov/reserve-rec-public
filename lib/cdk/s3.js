...
function s3Setup(scope, props) {
  ...
  const reserveRecPublicDistBucket = new s3.Bucket(scope, props.env.STACK_NAME + 'DistBucket', {
  ...
  const reserveRecPublicLogBucket = new s3.Bucket(scope, props.env.STACK_NAME + 'PublicLogBucket', {
