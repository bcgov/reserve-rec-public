...
const defaults = {
  config: {
    ...
    // Set to 'false' once the front door (frontDoorStack, /dayuse) is the only public
    // origin. The distribution stays in the stack (the front door mounts this stack's
    // dist bucket and the Mode 2 function lives here); it just stops serving. Login via
    // this origin cannot complete: COGNITO_REDIRECT_URI points at the front door, so
    // Amplify rejects the callback with InvalidOriginException.
    legacyDistributionEnabled: 'true',
...
class DistributionStack extends BaseStack {
  constructor(scope, primer) {
    ...
    this.publicDistribution = new cloudfront.Distribution(this, this.getConstructId('publicDistribution'), {
      ...
      enabled: this.getConfigValue('legacyDistributionEnabled') !== 'false',
    ...
    // Consumed by frontDoorStack, which mounts this bucket as the /dayuse tenant origin.
    this.exportReference(this, 'distBucketName', this.distBucket.bucketName, `Name of the SPA dist bucket in ${this.stackId}`);
  }
