...
const defaults = {
  config: {
    ...
    // Optional CLOUDFRONT-scope WAF WebACL. Provisioned out of band by
    // scripts/front-door-waf/provision_waf.py (LZA SCP blocks us-east-1 CFN).
    // Prefer webAclArnSSMPath (the script writes the ARN there); webAclArn is a
    // direct-value fallback. Empty = no WAF attached.
    webAclArnSSMPath: '',
    webAclArn: '',
  },
  constructs: {
...
class FrontDoorStack extends BaseStack {
  constructor(scope, primer) {
    ...
    // Resolve from SSM when a path is configured, else the direct-value fallback.
    const webAclArnSSMPath = this.getConfigValue('webAclArnSSMPath') || '';
    const webAclArn = webAclArnSSMPath
      ? resolveParameterFromSSM(this, webAclArnSSMPath)
      : (this.getConfigValue('webAclArn') || '');
    if (webAclArn) {
    ...
    // The public-facing base (host + tenant prefix, no scheme) for user-visible URLs —
    // booking-email links, Cognito redirects, etc. Consumed by reserve-rec-api via the
    // core stack's publicDomainNameSSMPath config.
    const publicFrontendBase = Fn.join('', [
      hasVanity ? domainNames[0] : this.frontDoorDistribution.domainName,
      '/dayuse',
    ]);
    this.exportReference(this, 'publicFrontendBase', publicFrontendBase, `Public frontend base (host/prefix) served by the Front Door in ${this.stackId}`);

    // The /dayuse* viewer-request function name. CloudFront allows one viewer-request
    // function per behavior, so the waiting-room Mode 2 gate shares this function with
    // the SPA fallback: the admin toggle-mode2 Lambda (reserve-rec-api) swaps its code
    // between fallback-only and gate+fallback at runtime. NOTE: a CDK deploy of this
    // stack resets the code to fallback-only — same accepted tradeoff as the standalone
    // distribution's WaitingRoomViewerFn.
    this.exportReference(this, 'dayuseViewerFnName', this.dayuseSpaFallbackFn.functionName, `Name of the /dayuse* viewer-request fn (SPA fallback + Mode 2 gate target) in ${this.stackId}`);
  }
}
