...
class ReserveRecPublicStack extends Stack {
  constructor(scope, id, props) {
    ...
    // Import existing API Gateway - use environment-specific export names
    const environment = props.env.ENVIRONMENT.toLowerCase();
    const reserveRecApiGatewayId = Fn.importValue(`reserveRecApi-${environment}-publicApiStack-publicApiId`);

    // Declare imported API Gateway (rootResourceId not needed for CloudFront origin)
