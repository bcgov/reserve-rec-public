    ...
    new cloudwatch.Dashboard(this, this.getConstructId('edgeDashboard'), {
      ...
      widgets: [
        ...
        [
          ...
          edge('Autoblock candidates (producer log)', ['parse @message /^\\s+(?<verdict>BLOCK|WATCH)\\s+(?<ip>\\S+)\\s+(?<family>\\S+)\\s+(?<reason>.*)$/', 'filter ispresent(verdict)', 'fields @timestamp, verdict, ip, family, reason', 'sort @timestamp desc', 'limit 20'], { region: this.region, logGroupNames: [`/aws/lambda/${this.getAppName()}-${env}-WafAutoblockDetector`] }),
        ],
      ],
    });
