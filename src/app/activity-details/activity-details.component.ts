...
export class ActivityDetailsComponent implements OnInit, AfterContentChecked, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Re-fetch with startDate when the date selection changes so waitingRoomActive reflects the chosen date
    this.dateRangeSub = this.form.get('dateRange')!.valueChanges.subscribe(value => {
      const startDate = Array.isArray(value) ? value[0] : null;
      this.activityService.getActivity(this.collectionId, this.activityType, this.activityId, false, startDate || undefined);
    });
  }
  ...
  isSubmitDisabled() {
    ...
    if (this.isSubmitting() || !this.form?.controls?.['dateRange']?.value || total === 0 || total > 10) {
      return true;
    }
 ...
 submit(): void {
    ...
    if (this.data?.waitingRoomActive) {
